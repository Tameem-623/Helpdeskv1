<x-app-layout>
    ...
    <div class="mb-4 flex justify-between">
        ...
        <a class="rounded-lg border border-transparent bg-custom-color px-4 py-2 text-center text-sm font-medium leading-5 text-white transition-colors duration-150 hover:bg-purple-700 focus:outline-none focus:ring active:bg-purple-600" href="{{ route('tickets.create') }}">
        {{ __('Create') }}
        </a>
                            ...
                            <tr class="text-gray-700">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @foreach($ticket->categories as $category)
                                        <span class="rounded-full  px-2 py-2">{{ $category->name }}</span>
                                    @endforeach
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    @foreach($ticket->labels as $label)
                                        <span class="rounded-full  px-2 py-2">{{ $label->name }}</span>
                                    @endforeach
                                </td>
                                @hasanyrole('admin|agent')
                                    <td class="px-4 py-3 text-sm">
                                ...
                                @endhasanyrole
                                <td class="px-4 py-3 space-x-0">
                                    @hasanyrole('admin|agent')
                                        <a class="rounded-lg border-2 bg-custom-color border-transparent bg-purple-600 px-4 py-1 text-center text-sm font-medium leading-5 text-white transition-colors duration-150 hover:bg-purple-700 focus:outline-none focus:ring active:bg-purple-600" href="{{ route('tickets.edit', $ticket) }}">
                                            {{ __('Edit') }}
                                        </a>
                                    @endhasanyrole
<div class="py-1">
                                    ...
                                    @role('admin')
                                        <form action="{{ route('tickets.destroy', $ticket) }}" method="POST" onsubmit="return confirm('Are you sure?')" style="display: inline-block;">
                                            @csrf
                                            @method('DELETE')
                                            <x-primary-button class="bg-custom-color py-1">
                                                Delete
                                            </x-primary-button>
                                        </form>
                                    @endrole
